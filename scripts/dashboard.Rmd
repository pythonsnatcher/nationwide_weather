...
library(RSQLite)  # Add this library

# Replace the data source section
query_weather_data <- function() {
  url <- "https://raw.githubusercontent.com/pythonsnatcher/nationwide_weather/5cce1ae87441d5fefdddb0e4d99b05dddb0e4d99b05ddde8d457a/data/nationwide_weather.db"
  temp_db <- tempfile(fileext = ".db")
  download.file(url, temp_db, mode = "wb")
  
  con <- dbConnect(SQLite(), temp_db)
  df <- dbGetQuery(con, "SELECT * FROM weather_data")
  dbDisconnect(con)
  unlink(temp_db)
  return(df)
}
...
df <- tryCatch({
  ...
  data.frame(Error = "Failed to load data from URL")
})
